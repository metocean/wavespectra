...
class TestJonswap(unittest.TestCase):
    ...
    def test_jonswap_series(self):
        ...
        for i, spec in enumerate(dset['efth']):
            assert_array_almost_equal(integrate_2d_hs(dset['freq'][:], dset['dir'][:], spec[:, :]), self.hs(tp[i], 0.01), decimal=3)


    def test_jonswap_matrix(self):
        tp = 10*np.random.random((5, 4, 3))
        dp = 360*np.random.random((5, 4, 3))
        dset = jonswap(tp, dp, alpha=0.01, dspr=25,
                       coordinates=[('time', np.arange(0, 5)), ('lat', np.arange(0, 4)), ('lon', np.arange(0, 3))])
        if plot:
            plt.pcolormesh(dset['freq'][:], dset['dir'][:], dset['efth'][0, :, :].T)
            plt.show()
            plt.pcolormesh(dset['freq'][:], dset['dir'][:], dset['efth'][1, :, :].T)
        ...
        assert_array_almost_equal(integrate_2d_hs(dset['freq'][:], dset['dir'][:], dset['efth'][i, j, k, :, :]), self.hs(tp[i, j, k], 0.01), decimal=3)
...
class TestOchiHubble(unittest.TestCase):
    ...
    def test_oh_scalar(self):
        ...
        if plot:
            plt.pcolormesh(dset['freq'][:], dset['dir'][:], dset['efth'][:, :].T)
            plt.show()
        assert_array_almost_equal(integrate_2d_hs(dset['freq'][:], dset['dir'][:], dset['efth'][:, :])
    ...
    def test_oh_series(self):
        ...
        for i, spec in enumerate(dset['efth']):
            ...
            print(params)
            assert_array_almost_equal(integrate_2d_hs(dset['freq'][:], dset['dir'][:], spec[:, :]),
