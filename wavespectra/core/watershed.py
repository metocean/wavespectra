...
def spec_partition(spec, ihmax=200):
...
if __name__ == '__main__':
    ...
    dset2 = read_ww3('/data/ww3/ec_glob-st4/ww320180806_00z/spec20180806T00_spec.nc').load()

    neigh = ptnghb(25, 24)
    for isite in range(10):
        print(isite)
        specs = dset2.efth[:, isite].values
        for isnap in range(193):
            spec = specs[isite]
            startTime = datetime.now()
            part_array0 = specpart.partition(spec)
            print(datetime.now() - startTime)

            startTime = datetime.now()
            part_array = spec_partition(spec, neigh=neigh, ihmax=200)
            print(datetime.now() - startTime)

    # figure(); pcolor(spec); colorbar()
    # figure(); pcolor(imi.reshape(spec.shape)); colorbar()
    # figure(); pcolor(imo.reshape(spec.shape)); colorbar()
    # figure(); pcolor(imd.reshape(spec.shape)); colorbar()
    # figure(); pcolor(part_array0); colorbar()
    # figure(); pcolor(part_array); colorbar()
    # figure(); pcolor(part_array-part_array0); colorbar()
